...
addLayer("l", {
   ...
   doReset(resettingLayer) {
       let keep = [];
       if (hasMilestone("b", 51) && resettingLayer == "b")
...
milestones: {
   ...
   buyables: {
       21: {
           cost(x) { return new Decimal(1).mul(new Decimal(x).mul(x)).add(1)},
           title() {
                           return format(getBuyableAmount(this.layer, this.id), 0) + "<br/>Poster Ads"
                       },
           display() { return "boosts money gain based on amount owned.<br>" + "<br>cost:" + this.cost() + "<br>" + format(buyableEffect(this.layer, this.id))+"x" },
           canAfford() { return player[this.layer].points.gte(this.cost()) },
           effect(x) {
       ...
       22: {
           cost(x) { return new Decimal(2).mul(new Decimal(x).mul(x)).add(1)},
           title() {
                           return format(getBuyableAmount(this.layer, this.id), 0) + "<br/>Newspaper Ads"
                       },
           display() { return "boosts money gain based on amount owned.<br>" + "<br>cost:" + this.cost() + "<br>" + format(buyableEffect(this.layer, this.id))+"x" },
           canAfford() { return player[this.layer].points.gte(this.cost()) },
           effect(x) {
            return getBuyableAmount(this.layer, this.id).add(1).pow(.7)
            },
           effectDisplay() { return format(buyableEffect(this.layer, this.id))+"x" },
           buy() {
               player[this.layer].points = player[this.layer].points.sub(this.cost())
       ...
       23: {
           cost(x) { return new Decimal(4).mul(new Decimal(x).mul(x)).add(1)},
           title() {
                           return format(getBuyableAmount(this.layer, this.id), 0) + "<br/>Television Ads"
                       },
           display() { return "boosts money gain based on amount owned.<br>" + "<br>cost:" + this.cost() + "<br>" + format(buyableEffect(this.layer, this.id))+"x" },
           canAfford() { return player[this.layer].points.gte(this.cost()) },
           effect(x) {
            return getBuyableAmount(this.layer, this.id).add(1).pow(.85)
            },
           effectDisplay() { return format(buyableEffect(this.layer, this.id))+"x" },
           buy() {
               player[this.layer].points = player[this.layer].points.sub(this.cost())
       ...
       24: {
           cost(x) { return new Decimal(8).mul(new Decimal(x).mul(x)).add(1)},
           title() {
                           return format(getBuyableAmount(this.layer, this.id), 0) + "<br/>Global Multimedia Advertising Campaign"
                       },
           display() { return "boosts money gain based on amount owned.<br>" + "<br>cost:" + this.cost() + "<br>" + format(buyableEffect(this.layer, this.id))+"x" },
           canAfford() { return player[this.layer].points.gte(this.cost()) },
           effect(x) {
...
addLayer("m", {
   ...
   gainMult() {
           ...
           if (hasUpgrade('t', 105)) mult = mult.times(3)
